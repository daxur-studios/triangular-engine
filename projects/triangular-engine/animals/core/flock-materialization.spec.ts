...
describe('materializeFlock', () => {
  const definition = { id: 'birds', seed: 7, origin: { x: 0, y: 5, z: 0 }, count: 3, spacing: 4, speed: 2, cullDistance: 10, hysteresis: 3 };
  ...
  it('is deterministic and culls beyond the residency radius', () => {
    ...
    expect(materializeFlock(definition, { ...observer, position: { x: 20, y: 0, z: 0 } })).toEqual([]);
  });
});
