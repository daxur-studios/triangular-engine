...
- **Follow-up fix — clipmap ring-skip check:** `clipmap-layout.ts`'s original "fullyCovered" check tested each candidate box against individual previously-placed tile boxes, which a coarser tile can never be a strict superset of (it never fired), so every level rendered across its *entire* block instead of just an outer ring — all 4 levels sat at `MAX_INSTANCES_PER_LEVEL` (64) simultaneously. Fixed by tracking one running bbox per level (that level's own full theoretical footprint) instead of per-tile boxes; a coarser level's footprint always contains every finer level's footprint by construction (tile size doubles per level, tile-count radius stays fixed) — verified numerically across 200k random camera offsets with a comfortable margin before relying on it. Instance counts now show a real outer ring (e.g. L0:64 · L1:48 · L2:48 · L3:55, varying with camera position) instead of all levels maxed out. Re-ran the crack-freeness check against the fixed layout across 500 random camera positions x all 3 level boundaries (12,744 edge-pair checks): zero coverage holes, zero gaps. Draw calls unaffected (still 4).
...
**clipmap-far-coverage-spike — Spike 4 (submission cost/draw-call bound), far-coverage half only**

- **Fixture:** `projects/demo-app/src/app/clipmap-far-coverage-spike` — same shared clipmap mechanism as `gpu-morph-lod-spike`, now factored into `triangular-engine/terrain/clipmap` (`createClipmapTerrainScene`, parameterized via a new optional `IClipmapTerrainSceneOptions` so both spikes share one implementation) and extended to 12 LOD levels, base tile 16 m, block radius 4 tiles, grid resolution 32 → outer ring ≈131,072 m. Tests only the far-coverage half of Candidate B / Spike 4's submission-cost question (near-field crack-freeness is the already-passed spike above); per-instance transform/culling/pass-multiplication cost accounting is not covered here.
- **Enabling change — stride clamping:** `clipmap-grid-geometry.ts` originally required `GRID_RESOLUTION` divisible by `2^(levelCount-1)`, which forces the base resolution to explode as level count grows. Replaced with `stride = min(2^level, resolution)`: levels at or beyond `log2(resolution)` clamp to a single quad per tile instead of demanding more resolution than exists. Lets level count scale independently of near-field triangle budget.
- **Draw-call sweep:** camera swept from 30 m out past 1.2x the outer ring (157 km); draw calls held at 12 (one `InstancedMesh` per level) at every sample, within Spike 4's ≤12-far bound. Frozen-camera stability check (120 frames) showed zero draw-call/triangle flicker.
- **Bug found — analytic noise precision loss at horizon scale:** the fbm/hash-based `'noise'` terrain kind's `fract(p * constants)` hashing loses usable precision in single-precision GLSL floats well before 100 km, degenerating into flat bands/spikes. This is a limitation of the analytic placeholder height function, not of the clipmap ring-extension mechanism. Worked around by defaulting the spike to the `'wave'` (sin/cos) terrain kind, which stays stable at these coordinates; `'noise'` is kept as a toggle to demonstrate the limit directly. Any real height-sampling replacement (e.g. the heightmap-texture path landing concurrently in `clipmap-terrain-material.ts`) is not subject to this limit.
- **Bug found and fixed — border-blend formula incompatible with stride clamping:** the vertex shader's `borderCoarseCell` (the neighbour-level cell size used to snap a fine tile's edge vertices onto its coarser neighbour's grid, per the crack-freeness fix above) computed it as `vertexSpacingM * 4^(level+1)`, assuming vertex spacing compounds indefinitely as tile size and stride both double per level. Once the stride clamp made real spacing flatten to `baseTileSizeM * 2^level` for levels ≥ log2(resolution), this formula overshot real spacing by an increasing factor at high levels, collapsing `borderBlend` to ≈1 almost everywhere on far tiles and snapping visually distinct vertices onto a handful of shared oversized grid points — reported by visual check as severe corruption ("everything collapsing near the center": nested/overlapping wireframe diamonds, dense point-clustering, starburst artifacts). Fixed with a `realVertexSpacingM(level)` GLSL helper that mirrors the CPU-side clamp exactly, used in place of the old `pow(4.0, ...)` formula. Confirmed fixed by visual re-check (wireframe: clean nested LOD rings doubling outward as expected; shaded: coherent rolling terrain at all sweep distances, no clustering).
- **Threshold:** Spike 4's far-coverage draw-call bound (≤12) held throughout the sweep; visual detail at the outer ring is legible (wave terrain, no popping observed at the tested fly-to distances) with the caveat that a production height function's own precision behavior at this scale is untested here.
- **Remaining uncertainty:** submission cost beyond draw-call count (actual per-instance transform/upload cost, visibility culling, pass multiplication) is unmeasured — this spike only checks the draw-call bound and visual coherence, not full submission-cost accounting per Spike 4's complete hypothesis. Cubesphere 2:1 and explicit-feature-patch boundary fixtures remain untested (as with the near-boundary spike).
- **Result: PASSED** for the scope this spike's hypothesis claims (LOD ring extension to horizon scale keeps draw calls bounded to one InstancedMesh per level; outer-ring visual detail acceptable once given a scale-stable height function). Status updated in `pages/spikes-index/spikes-index.component.ts`.
