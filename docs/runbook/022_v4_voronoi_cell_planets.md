...
- State: In progress — **M0 (graph core) and M1 (tectonics) implemented**, isolated POC, not integrated with existing terrain/CDLOD; M1 is now wired into the `/cell-planet-lab` lab page's 2D unwrap view (plates/elevation/land-water color modes) as an early visual gut-check, ahead of the M3 debug lab proper
- Date: 2026-08-26
